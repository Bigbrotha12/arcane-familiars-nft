...
async function fetchOwnedFamiliars(
  accountKey: string | undefined,
  db: D1Database,
): Promise<Response> {
  ...
  const res = await app.fetch(
    new Request("https://example.com/game/owned-familiars"),
    makeEnv(db),
    ctx,
  );
...
describe("GET /game/owned-familiars response contract", () => {
  ...
  it("returns synced:true and the stubbed familiar list when a wallet is bound", async () => {
    ...
    await db
      .prepare("DELETE FROM wallet_bindings WHERE sub = ?")
      .bind("sub-bound")
      .run();
    await db
      .prepare(
        "INSERT OR REPLACE INTO wallet_bindings (sub, wallet_address) VALUES (?, ?)",
      )
