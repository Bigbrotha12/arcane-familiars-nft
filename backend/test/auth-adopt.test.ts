...
async function seedGameState(
  ...
  await env.DB
    ...
    .bind(
      anonymousId,
      JSON.stringify({ version: 1, anonymousId }),
      isAnonymous ? 1 : 0,
    )
    .run();
}
...
async function gameStateRow(anonymousId: string): Promise<GameStateRow | null> {
  return env.DB
    .prepare("SELECT anonymous_id, state_json, version, is_anonymous FROM game_states WHERE anonymous_id = ?")
    .bind(anonymousId)
    .first<GameStateRow>();
}
...
async function battleRow(battleId: string): Promise<{ anonymous_id: string } | null> {
  return env.DB
    .prepare("SELECT anonymous_id FROM active_battles WHERE battle_id = ?")
    .bind(battleId)
...
async function dungeonRunRow(runId: string): Promise<{ anonymous_id: string } | null> {
  return env.DB
    .prepare("SELECT anonymous_id FROM dungeon_runs WHERE id = ?")
    .bind(runId)
    .first<{ anonymous_id: string }>();
}
