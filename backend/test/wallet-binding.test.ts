...
const accountA = privateKeyToAccount('0x4c0883a69102937d6231471b5dbb6204fe5129617082792ae468d01a3f362318');
const accountB = privateKeyToAccount('0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d');

function makeEnv(): Bindings {
  return { ...(env as unknown as Bindings), ENVIRONMENT: 'production' };
}

async function fetchAuth(path: string, method: string, token: string | null, body: unknown): Promise<Response> {
...
describe('wallet binding nonce flow', () => {
  ...
  it('binds a wallet after signing the exact server-issued message', async () => {
    ...
    const row = await env.DB.prepare('SELECT wallet_address FROM wallet_bindings WHERE sub = ?')
      .bind('sub-wallet-a')
