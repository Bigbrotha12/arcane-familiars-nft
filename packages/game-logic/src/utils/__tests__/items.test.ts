...
describe("item data registry", () => {
  it("exposes all ten basic items", () => {
    expect(Object.keys(ITEMS).sort()).toEqual(
      ["elixir", "ether_large", "ether_small", "fire_bomb", "frost_bomb", "lucky_coin", "phoenix_feather", "potion_medium", "potion_small", "tonic"].sort(),
    );
  });
...
describe("item effects in battle", () => {
  ...
  it("damage items hit the enemy, not the user", () => {
    ...
    const { playerResult, updatedEnemyFamiliar, updatedPlayerFamiliar } = useItem(getItem("fire_bomb")!, makePlayer(), enemy);
  ...
  it("tonic cleanses debuffs and dots but keeps buffs", () => {
    const debuff = { abilityId: "shadowstrike", type: EffectType.Debuff, stat: StatName.Defense, value: 0.8, turnsRemaining: 2 };
