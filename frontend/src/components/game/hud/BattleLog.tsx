...
// Interactive so the player can scroll; anchored right side by BattleHUD.
  ...
  return (
    <div className="pointer-events-auto flex max-h-40 w-60 flex-col overflow-hidden rounded-md bg-[#1E1B4B]/85 shadow-card backdrop-blur-sm">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-sm py-1.5">
        {entries.length === 0 ? (
          <p className="font-mono text-xs text-[#8F8BBF]">No battle events yet.</p>
        ) : (
          // Index keys are correct here because BattleScene's log is append-only
          // (newest last, capped at 12 entries). If the log ever supports
