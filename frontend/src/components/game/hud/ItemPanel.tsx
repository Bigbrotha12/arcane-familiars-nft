  ...
  return (
    <Modal open={open} onClose={onClose} title="Select Item" hud compact>
      <div className="flex flex-col gap-2">
        {items.length === 0 && <p className="font-body text-xs text-text-muted">No items available.</p>}
        {items.map((item) => {
          ...
          return (
            <div key={item.id} className="flex flex-col gap-0.5">
              ...
              >
                ...
                <span className="font-mono text-[10px] tabular-nums text-text-secondary">x{item.quantity}</span>
              </Button>
