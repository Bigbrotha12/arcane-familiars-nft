  ...
  return (
    <Modal open={open} onClose={onClose} title="Select Familiar" hud>
      <div className="flex flex-col gap-3">
        {party.length === 0 && <p className="font-body text-sm text-text-muted">No familiars in your party.</p>}
        {party.map((familiar) => {
              ...
              >
                ...
                <span className="font-mono text-xs tabular-nums">
                  <span className="text-teal">
                    HP {Math.max(0, Math.floor(familiar.hp))}/{familiar.maxHp}
                  </span>
                  <span className="ml-2 text-accent">
                    MP {Math.max(0, Math.floor(familiar.mp))}/{familiar.maxMp}
                  </span>
                </span>
              </Button>
              {isActive && <p className="px-2 text-sm text-text-muted">Currently in battle</p>}
              {isFainted && !isActive && <p className="px-2 text-sm text-text-muted">Fainted</p>}
            </div>
          );
        })}
      </div>
