  ...
  return {
    ...
    resolveId(source, importer) {
      ...
      if (importer?.startsWith(gameSrc)) {
        const candidates = [path.resolve(gameSrc, rel), path.resolve(gameLogicSrc, rel)];
...
export default defineConfig(({ mode }) => ({
  plugins: [
    ...
    resolveGameAtAlias(),
    nodePolyfills({
      include: ['crypto', 'buffer', 'process', 'stream', 'util', 'events', 'http', 'https', 'url', 'zlib'],
    }),
  ],
  resolve: {
    alias: [{ find: '@/game', replacement: path.resolve(gameSrc, 'index.ts') }],
  },
  server: {
    port: 8080,
    open: true,
